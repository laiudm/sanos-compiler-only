...
Sym *sym_push2(Sym **ps, int v, int t, int c) {
  Sym *s;
  s = sym_malloc();
  s->v = v;			//dcm: symbol token
  s->type.t = t;	//dcm: associated type
  s->c = c;			//dcm: associated number
  s->next = NULL;	//dcm: next related symbol
  // Add in stack
  s->prev = *ps;	//dcm: previous symbol in stack
  *ps = s;			//dcm: point head of stack to this latest entry
  return s;
}

// Find a symbol and return its associated structure. 's' is the top of the symbol stack
Sym *sym_find2(Sym *s, int v) {	//dcm: s is the tail of a linked list of symbols
  while (s) {
    if (s->v == v) return s;
    s = s->prev;
  }
