...
static int64_t getclock_us(void) {	// dcm: intriguing - could have put gettimeofday() into sow.c & got rid of this platform-specific code
#ifdef _WIN32
  struct _timeb tb;
  _ftime(&tb);
...
int parse_arguments(TCCState *s, int argc, char **argv) {
  ...
  while (1) {
    ...
    r = argv[oind++];
    if (r[0] != '-' || r[1] == '\0') {	// dcm: all options must start with "-", otherwise its a new file
      // Add a new file
    ...
    option_found:
      if (popt->flags & TCC_OPTION_HAS_ARG) {
        ...
        if (*r1 != '\0') goto show_help;	// dcm: could just as easily call help() & exit(1) here.
        oarg = NULL;
      }
      ...
      switch (popt->index) {
        ...
        case TCC_OPTION_D: {
          ...
          tcc_define_symbol(s, sym, value); // dcm: defined in compiler.c
          break;
        }
        case TCC_OPTION_U:
          tcc_undefine_symbol(s, oarg);		// dcm: defined in compiler.c
          break;
        case TCC_OPTION_L:
          tcc_add_library_path(s, oarg);	// dcm: defined in compiler.c
          break;
        case TCC_OPTION_B:
        ...
        case TCC_OPTION_g:
          do_debug = 1;
          break;
        case TCC_OPTION_c:			// dcm:  compile only; generate an object file
          multiple_files = 1;
          output_type = TCC_OUTPUT_OBJ;
          break;
        case TCC_OPTION_nofll:		// dcm:  undocumented.
          s->nofll = 1;
          break;
        ...
        case TCC_OPTION_r:
          ...
          output_type = TCC_OUTPUT_OBJ;
          break;
        case TCC_OPTION_noshare:	// dcm: undocumented
          s->noshare = 1;
          break;
        case TCC_OPTION_nostdinc:	// dcm: undocumented
          s->nostdinc = 1;
          break;
        case TCC_OPTION_nostdlib:	// dcm: undocumented
          s->nostdlib = 1;
          break;
        case TCC_OPTION_print_search_dirs:	// dcm: undocumented
          print_search_dirs = 1;
          break;
        case TCC_OPTION_v:
          do {
        ...
        case TCC_OPTION_Wl: {
          ...
          if (strstart(oarg, "-Ttext,", &p)) {
            ...
            if (strstart(p, "elf32-", NULL)) {
              s->output_format = TCC_OUTPUT_FORMAT_ELF;		// dcm: documented very lightly in cc.h
            } else if (!strcmp(p, "binary")) {
              s->output_format = TCC_OUTPUT_FORMAT_BINARY;	// dcm: doesn't appear to be tested anywhere, there are tests for TCC_OUTPUT_FORMAT_ELF though.
            } else {
...
int main(int argc, char **argv) {
  ...
  char *alt_lib_path;
  //dcm: test for syntax edge case. The standard allows this - it redefines i too. The comma doesn't separate expns.
  //for (int b = 0, i = 1; i < 5; i++) {
  //	  warning("testing syntax %i", b);
  //}

  init_util();		// dcm: just sets up a speedy ch lookup table.
  s = tcc_new();	// dcm:  defined in compiler.c
  ...
  if (output_type == TCC_OUTPUT_PREPROCESS) {
    ...
    if (!outfile) {
      ...
      ext = tcc_fileextension(objfilename);	// dcm: ext is a pointer into the buffer, so strcpy()'s below overwrite the buffer.
  ...
  tcc_set_output_type(s, output_type);	// dcm: does quite a bit of setup too.
  ...
  for (i = 0; i < nb_files && ret == 0; i++) {
    ...
    } else if (filename[0] == '-' && filename[1]) {		// dcm: libraries still have -l switch prefix so remove here
  ...
  if (do_bench) {
    ...
    if (total_bytes < 1) total_bytes = 1;	// dcm: total_bytes updated by preproc.c
