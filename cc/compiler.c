...
int parse_btype(CType *type, AttributeDef *ad) {
  ...
  typespec_found = 0;
  typedef_found = 0;
  t = 0;		//dcm: a bitmask of the type. Lowest bits (VT_BTYPE) are a field giving the basic type: char, void, etc
  while (1) {
    switch (tok) {
      case TOK_EXTENSION:
      ...
      case TOK_INT:
        next();
        typespec_found = 1;		//dcm: don't understand why u isn't set & checked for "too many basic types". Is int the default?
        break;
      ...
      case TOK_LONG:
        next();
        if ((t & VT_BTYPE) == VT_DOUBLE) {			//dcm: deal with "double long" and "long long"
          t = (t & ~VT_BTYPE) | VT_LDOUBLE;
        } else if ((t & VT_BTYPE) == VT_LONG) {
          t = (t & ~VT_BTYPE) | VT_LLONG;
        } else {
          u = VT_LONG;
          goto basic_type1;			//dcm: weird. could harmonise with other cases: remove next() & jump to basic_type. Nope - this is just one path
        }
        break;				//dcm: okay, I guess can't jump to basic_type as error checking will fail?
      ...
      case TOK_DOUBLE:
        ...
        break;		//dcm:  similar logic to TOK_LONG processing above.
...
void type_decl(CType *type, AttributeDef *ad, int *v, int td) {
  ...
  //dcm: parse the following portion of the declarator:{{*}} {{type-qualifier}}, where type-qualifier: const restrict volatile
  //dcm: interesting that qualifiers are reset on any *
  while (tok == '*') {
    qualifiers = 0;
  redo:
    next();
    ...
    mk_pointer(type);		//dcm: a * has been found.
    type->t |= qualifiers;
  }
  ...
  if (tok == '(') {					//dcm: deal with 6.7.5:  direct-declarator: (declarator) -- for pointer to arrays & pointer to fn (see https://en.cppreference.com/w/c/language/declarations)
    next();
    // TODO: this is not correct to modify 'ad' at this point, but the syntax is not clear
    parse_modifiers(ad);
    type_decl(&type1, ad, v, td);	//dcm: recursive call
    skip(')');
  } else {							//dcm: deal with 6.7.5:  direct-declarator: identifier
    // Type identifier
  ...
  type2 = &type1;		//dcm: I'm nervous - address of a local var is being taken, which will go out of scope on return. Is this okay?
						//dcm: checked latest version of this code (https://repo.or.cz/tinycc.git/blob/refs/tags/release_0_9_27:/tccgen.c#l4401)
						//dcm: but there are lots of changes so can't tell....
  for (;;) {
    s = type2->ref;
    type2 = &s->type;
    if (!type2->t) {
 ...
 tok_next:
  switch (tok) {
    ...
    default:
      ...
      vset(&s->type, r, s->c);			//dcm: is this where straight variable access is handled?
  ...
  while (1) {
    if (tok == TOK_INC || tok == TOK_DEC) {
      ...
      next();
    } else if (tok == '[') {		//dcm: array accessing
      next();
      gexpr();
      gen_op('+');
      indir();
      skip(']');
    } else if (tok == '(') {		//dcm: wow, very late in the processing to be dealing with function calls.
      SValue ret;
      Sym *sa;
      int nb_args;
      ...
      if (tok != ')') {		//dcm: parse the function's arguments
        for (;;) {
          expr_eq();
...
void expr_eq(void) {
  ...
  if (const_wanted) {
    ...
    expr_lor();				//dcm: parse the 1st operand 
    if (tok == '?') {		//dcm: 6.5.15 conditional operator/ternary processing <expr_lor> ? <gexpr> : <expr_eq> | <expr_lor> ?  : <expr_eq> (not valid!)
      next();
      if (vtop != vstack) {
      ...
      if (tok == ':') {		//dcm: check for empty 1st expr: <expre_lor> ? : <expr_eq>. Is this even valid?
        gv_dup();
        tt = gtst(1, 0);
      } else {				//dcm: typical path
        tt = gtst(1, 0);
        gexpr();			//dcm: parse the 2nd operand
      }
      ...
      u = gjmp(0, 0);
      gsym(tt);
      expr_eq();			//dcm: parse the 3rd operand (C std says this should be a conditional-expression)
      type2 = vtop->type;
	  ...
	  //dcm: since code is output serially, it looks like it must jump forward to the code that does the casting. Yuck.
      tt = gjmp(0, 0);
      gsym(u);
...
void gexpr(void) {
  while (1) {
    expr_eq();
    if (tok != ',') break;
    vpop();			//dcm: discard the expression result for all but the last stmt
    next();
  }
}
...
void block(int *bsym, int *csym, int *case_sym, int *def_sym, int case_reg, int is_expr) {
  ...
  if (tok == TOK_IF) {
    ...
    gsym(a);
    gsym_at(b, d);
  } else if (tok == '{') {	//dcm: come here for nested compound statement.
    Sym *llabel;
    ...
    *def_sym = glabel();
    ...
    block_after_label:
      ...
      // Expression case
	  //dcm: looks like assignments are handled here too.
      if (tok != ';') {
        if (is_expr) {
          vpop();
          gexpr();
...
//dcm: Primary call is from decl(), but also called twice from unary()
  ...
  for (;;) {
    ...
    for (sym = global_stack; sym != NULL; sym = sym->prev) {
      ...
      if (((type->t & VT_BTYPE) == VT_FUNC) &&
        ...
        gen_function(sym);		//dcm: eventually calls block()
...
int isDeclSingular(int l) {
  ...
  AttributeDef ad;
	...
	//dcm: only get here if we've already found and parsed the 
    //dcm: {{ storage-class-specifier | type-specifier | type-qualifier | fn-specifier }} portion of a declaration
	//dcm: Now time to parse the remainder: {{ declarator | declarator = initializer }}
    while (1) { 
      type = btype;
      type_decl(&type, &ad, &v, TYPE_DIRECT);

	  //dcm: the declarator all the way up to the optional "= initialiser" has now been parsed.

	  //dcm: ignore this bit - this is historical only
      if ((type.t & VT_BTYPE) == VT_FUNC) {
	  ...
	  //dcm: see if it's a function definition (as opposed to a function declaration - see https://en.cppreference.com/w/c/language/declarations)
      if (tok == '{') {
        ...
        if ((type.t & (VT_INLINE | VT_STATIC)) == (VT_INLINE | VT_STATIC)) {
          ...
          sym->r = VT_SYM | VT_CONST;
          gen_function(sym);		//dcm: down the rabbit hole - eventually calls block()
        }
        break;
      } else {
        if (btype.t & VT_TYPEDEF) {
		  ...
		  //dcm: we've processed the declaration up to the point that we know it's not a function or a typedef.
		  //dcm: it's now time to see if there's a "= initialiser" portion to the decl.
		  //dcm: i.e. We only check for an "=" if it's valid to expect one
          // Not lvalue if array
          r = 0;
          if (!(type.t & VT_ARRAY)) r |= lvalue_type(type.t);
          has_init = (tok == '=');		//dcm: finally - the "=" check
...
int tcc_compile(TCCState *s1) {
  ...
  if (setjmp(s1->error_jmp_buf) == 0) {
    ...
    next();
    decl(VT_CONST);		//dcm: parsing starts here.
...
void tcc_define_symbol(TCCState *s1, const char *sym, const char *value) {
  ...
  *bf->buf_end = CH_EOB;
  ...
  ch = file->buf_ptr[0];
  next_nomacro();	// dcm: defined in preproc.c
  parse_define();	// dcm: defined in preproc.c
  file = NULL;
}
...
TCCState *tcc_new(void) {
  ...
  while (*p) {
    ...
    ts = tok_alloc(p, r - p - 1);	// dcm: defined in preproc.c
    p = r;
  }

  // Add dummy defines for some special macros to speed up tests
  // and to have working defined()
  define_push(TOK___LINE__, MACRO_OBJ, NULL, NULL);	// dcm: defined in preproc.c
...
int tcc_add_file_ex(TCCState *s1, const char *filename, int flags) {
  ...
  if (flags & AFF_PREPROCESS) {
  ...
  } else {	//dcm: come here to load the libc.a file.
    fd = file->fd;
    ...
    if (ehdr.e_ident[0] == ELFMAG0 && ehdr.e_ident[1] == ELFMAG1 && ehdr.e_ident[2] == ELFMAG2 && ehdr.e_ident[3] == ELFMAG3) {
      ...
      file->line_num = 0; // Do not display line number if error
      ret = tcc_load_archive(s1, fd);		//dcm: loads libc.a here
...
int tcc_set_output_type(TCCState *s, int output_type) {	// dcm: only ever called from cc.c
  char buf[1024];
